...
- **Public analytics (after consent):** Public pages (`/`, `/catalog`, `/catalog/{id}`, `/invite`, `/privacy`, `/sources`) load Google Tag Manager and Google Analytics only after you Accept the cookie banner. Decline or dismiss keeps analytics off for that session. The container id comes from `OPEN_UX_GTM_ID` (default `GTM-N3BL3G9K`). No analytics on `/mcp`, `/admin`, `/invite/redeem`, `/account`, or `/health`.
