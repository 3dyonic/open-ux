#!/usr/bin/env node
// Adapted from cursor/plugin-template scripts/validate-template.mjs (MIT).
// Validates .cursor-plugin/marketplace.json and each plugin manifest under clients/.
  ...
  for (const [index, entry] of marketplace.plugins.entries()) {
    ...
    if (pluginManifest.hooks !== undefined) {
      ...
      if (!(await pathExists(hooksPath))) {
        addError(`${entry.name}: hooks declared in plugin.json but hooks/hooks.json is missing.`);
      }
    }
    ...
    if (pluginManifest.mcpServers !== undefined) {
      ...
      if (!(await pathExists(mcpPath))) {
        addError(`${entry.name}: mcpServers declared in plugin.json but mcp.json is missing.`);
      }
    }
  }
