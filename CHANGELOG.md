...
- **Plugin:** dropped `commands/list.md`, `get.md`, `pack.md`, `actions.md`, `forms.md`, `feedback.md`. The `open-ux` skill is now the plugin's only registered entry (was 7: skill + 6 commands). `actions`/`forms`/`feedback` were container aliases for `pack`'s `jobs=` parameter re-implemented as standalone commands with skill-shaped frontmatter, duplicating `cards.md` and costing every install a standing per-turn description load with no distinct behavior. `SKILL.md`'s Loop now folds in the vague-ask branch (`suggest_situations`) as step 0, so it no longer disagrees with the Route table's own "vague surface" guidance. `SKILL.md`'s frontmatter `description` is reworded for reliable auto-invocation on ordinary UI/UX phrasing rather than catalog-specific terms. New reference [`principles.md`](clients/plugin/skills/open-ux/principles.md) states the design rules an ambiguous ask or a proposed new tool/command should be checked against. `guideline.md` and `component.md` no longer carry vestigial skill-shaped frontmatter (they never registered as skills; the frontmatter only implied that they did).
