...
- Contributors, CI, and Docker install from `packages/mcp/uv.lock` via **uv**. End users still `pip install open-ux`. Hatch/PyPI package shape unchanged.

- **Plugin 1.2.1:** bare `/open-ux:open-ux` — fixed verbatim onboarding in [`bare-invoke.md`](clients/plugin/skills/open-ux/bare-invoke.md) every time (what it is, how it guides the LLM, tools/helpers one-liner, ask for task in next message).

- **Plugin:** dropped `commands/list.md`, `get.md`, `pack.md`, `actions.md`, `forms.md`, `feedback.md`. The `open-ux` skill is now the plugin's only registered entry (was 7: skill + 6 commands). `actions`/`forms`/`feedback` were container aliases for `pack`'s `jobs=` parameter re-implemented as standalone commands with skill-shaped frontmatter, duplicating `cards.md` and costing every install a standing per-turn description load with no distinct behavior. `SKILL.md`'s Loop now folds in the vague-ask branch (`suggest_situations`) as step 0, so it no longer disagrees with the Route table's own "vague surface" guidance. `SKILL.md`'s frontmatter `description` is reworded for reliable auto-invocation on ordinary UI/UX phrasing rather than catalog-specific terms; agent frontmatter aligned as a shorter echo. New reference [`principles.md`](clients/plugin/skills/open-ux/principles.md) states design intent, a full ask-first decision tree, and rules an ambiguous ask or proposed new tool/Card should be checked against; [`tools.md`](clients/plugin/skills/open-ux/tools.md) adds the nine-tool verb table and API ergonomics. MCP tool blurbs and `suggest_situations` note now document container broad-pass (`jobs=forms|actions|feedback`, no `situation` envelope). First-call wayfinding: `list_situations` response notes, richer `pack` missing-`jobs=` error, map/`pack` tool descriptions lead with **When:**, always-on rule names the first call. `guideline.md` and `component.md` no longer carry vestigial skill-shaped frontmatter (they never registered as skills; the frontmatter only implied that they did).
