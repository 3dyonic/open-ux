...
PUBLIC_PATHS = ("/", "/catalog", f"/catalog/{ANT_SEED}", "/invite")
NEVER_GET = (
    "/mcp",
...
def test_privacy_page_serves_privacy_doc(tmp_env: Path) -> None:
    doc = (Path(__file__).resolve().parents[3] / "docs" / "PRIVACY.md").read_text(
        encoding="utf-8"
    )
    with _client() as client:
    ...
    html = response.text
    assert "<h1>Privacy</h1>" in html
    assert "Never persist" in html
    assert "audit</code> does not take a file" in html
    assert "`audit` does not take a file" in doc
    assert "Google Tag Manager" in html
    assert "Google Analytics" in html
    assert "after you Accept the cookie banner" in html
    assert "No analytics on" in html
    assert "/mcp" in html
    assert GTM_HOST not in html
    assert DEFAULT_GTM_ID in html
    assert CONSENT_BANNER_COPY not in html
