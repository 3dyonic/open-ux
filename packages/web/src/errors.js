...
export function renderNotFound(root, detail = "", { kind = "page" } = {}) {
...
export function renderServerError(root, path = location.pathname || "/") {
  const href = path || "/";
  setTitle("This page could not be loaded — Open UX");
  root.innerHTML = shell(
    `
  <main class="page page-error">
    ${PIP_LOOK}
