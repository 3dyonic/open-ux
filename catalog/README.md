...
- `rules/` — 295 guideline files (filename = `id`). Seven leftovers with no honest Leaf were dropped. Same-claim rows fold onto one survivor; `citation` is always an array of `{source, url}`.
